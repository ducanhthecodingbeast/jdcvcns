...
def main():
    ...
    if not os.path.exists(jd_path) or cv_path is None:
        raise FileNotFoundError("Missing datasets. Please make sure Dataset/jd.csv and a non-empty mockcv.csv exist.")
    ...
    if store_db:
        ...
        try:
            def ranked_matches(cv_idx):
                cv_scores = hybrid_similarities[cv_idx]
                ...
                return [
                    {
                        ...
                        "meta": {
                            "bge_dense": float(bge_dense_similarities[cv_idx][int(jd_idx)]),
                            "lexical": float(sparse_similarities[cv_idx][int(jd_idx)]),
                        },
                    }
                    for jd_idx in top_k_idx
                ]
