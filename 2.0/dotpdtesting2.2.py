...
from html_reports import build_two_stage_hybrid_recommendations, render_cv_to_jd_report
...
def main():
    ...
    cv_cache_v2 = os.path.join(source_dir, "cv_embeddings_v2.npy")
    jd_cache_v2 = os.path.join(source_dir, "jd_embeddings_v2.npy")
    ...
    if store_db:
        ...
        try:
            ...
            for i, (_, cv_row) in enumerate(tqdm(df_cv.iterrows(), total=len(df_cv), desc="Writing CV matches")):
                ...
                cv_v2_scores = v2_similarities[i]
                top_30_idx = np.argsort(cv_v2_scores)[::-1][:30]
                bge_dense_scores = bge_dense_similarities[i]
                lexical_scores = sparse_similarities[i]
                top_k_scores = [
                ...
                matches = []
                for rank, (jd_idx, hybrid_score) in enumerate(top_k_scores[:top_k], start=1):
                    matches.append(
                        {
                            "jd_id": jd_ids[jd_idx],
                            "rank": rank,
                            ...
                            "meta": {
                                "jd_idx": jd_idx,
                                "v2": float(v2_similarities[i][jd_idx]),
                                "bge_dense": float(bge_dense_similarities[i][jd_idx]),
                                "lexical": float(sparse_similarities[i][jd_idx]),
                            },
                        }
                    )
    ...
    html_out = os.path.join(target_dir, "results_2.2_hybrid_dotproduct.html")
    recommendations = build_two_stage_hybrid_recommendations(
        v2_similarities,
        bge_dense_similarities,
        sparse_similarities,
        retrieve_k=30,
        top_k=10,
    )
    render_cv_to_jd_report(
        ...
        df_jd,
        recommendations,
        title="CV Matching Results - 2.2 Hybrid Dot Product",
        subtitle="Vietnamese_Embedding_v2 retrieval with BGE-M3 and TF-IDF reranking",
    )
