...
try:
    ...
    tokenizer_classes = [
        ...
        getattr(transformers, "PreTrainedTokenizer", None),
        getattr(transformers, "PreTrainedTokenizerFast", None),
        getattr(getattr(transformers, "tokenization_utils_tokenizers", None), "TokenizersBackend", None),
    ]
    for tokenizer_class in tokenizer_classes:
        if tokenizer_class is not None and not hasattr(tokenizer_class, "prepare_for_model"):
            tokenizer_class.prepare_for_model = monkey_patch_prepare_for_model
...
def select_cvs_by_title_similarity(
    ...
    *,
    ...
    title_texts = [clean_text(row[TITLE_COLUMN]) for _, row in sample_jds.iterrows()]
    desired_job_texts = [clean_text(row.get(DESIRED_JOB_COLUMN, "")) for _, row in df_cv.iterrows()]
    ...
    for title_idx, scores in enumerate(title_cv_scores):
        ...
        for rank, cv_idx in enumerate(np.argsort(scores)[::-1][:cvs_per_title], start=1):
            ...
            cv_row["source_jd_title"] = clean_text(jd_row[TITLE_COLUMN])
